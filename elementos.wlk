...
class Bloque {
	...
	method sePuedeEmpujarA(posicion) = utilidadesParaJuego.sePuedeMover(
		posicion
	) and (not nivelBloques.hayBloque(posicion))
	
	method empujar(posicion) {
		if (self.sePuedeEmpujarA(posicion)) self.position(posicion)
	}
}

object deposito {
	// esta determinado por la zona de baldozas color naranja
	method contiene(posicion) = posicion.x().between(
		5,
		9
	) and posicion.y().between(7, 12)
}

object salida {
	// la salida se visualiza siempre en el mismo lugar del tablero
	...
	method reaccionarA(unPersonaje) {
		...
		// No hace nada.
		// La victoria se evalúa desde PersonajeNivelLlaves.
	}
}
...
class Modificador inherits Elemento {
	...
	method efecto() = { unPollo, energiaActual => unPollo.energia() }
...
class Duplicador inherits Modificador {
	override method efecto() = { unPollo, energiaActual => unPollo.energia() * 2 }
	
	override method reaccionarA(unPersonaje) {
		super(unPersonaje)
		game.say(self, "Duplicador activado")
	}
}
...
class Reforzador inherits Modificador {
	...
	override method efecto() = { unPollo, energiaActual =>
		(unPollo.energia() * 2) + self.energiaExtra(energiaActual) }
	
	override method reaccionarA(unPersonaje) {
		super(unPersonaje)
		game.say(self, "Reforzador activado")
	}
}
...
class TripleOrNada inherits Modificador {
	...
	override method efecto() = { unPollo, energiaActual =>
		unPollo.energia() * self.multiplicador(energiaActual) }
	
	override method reaccionarA(unPersonaje) {
		super(unPersonaje)
		game.say(self, "Triple o nada")
	}
}
...
class CeldaSorpresa inherits Elemento {
	...
	override method esCeldaSorpresa() = true
